...
function QuotationsPage(){
 ...
 async function load(){try{setLoading(true);setError("");const [qs,ls]=await Promise.all([api.getQuotations(),api.getLeads()]);setQuotations(qs);setQualifiedLeads(ls.filter(l=>l.status.toUpperCase()==="QUALIFIED"));}catch(e){setError(e instanceof Error?e.message:"Unable to load quotations.");}finally{setLoading(false);}}
 useEffect(()=>{load();},[]);
