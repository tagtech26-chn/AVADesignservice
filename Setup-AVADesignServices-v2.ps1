...
Write-Step "3. Detecting ASP.NET Core Web API project"

$ProjectFile = Join-Path $Backend "backend-api.csproj"
if (Test-Path $ProjectFile) {
    Write-Host "Existing backend-api project found. Preserving the current project." -ForegroundColor Green
} else {
    Write-Host "backend-api project not found. Creating a new .NET 9 Web API project." -ForegroundColor Yellow
    Push-Location $Root
    try {
